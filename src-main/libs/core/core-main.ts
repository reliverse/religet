export { enableSecure, Peer } from "./core-impl/peer.js";
export type { PeerType, PeerSource } from "./core-impl/peer.js";
export {
  Selections,
  isLowerIntersecting,
  isUpperIntersecting,
...
import ConnPool from "./core-impl/conn-pool.js"; // browser exclude
import { NodeServer, BrowserServer } from "./core-impl/server.js";
import Torrent from "./core-impl/torrent.js";
...
export default class WebTorrent extends EventEmitter {
  constructor(opts = {}) {
    ...
    if (typeof opts.peerId === "string") {
      ...
      this.peerId = arr2hex(
        text2arr(VERSION_PREFIX + arr2base(randomBytes(9))),
      );
    }
    ...
    this._uploadLimit = Math.max(
      typeof opts.uploadLimit === "number" ? opts.uploadLimit : -1,
      -1,
    );
  ...
  createServer(options, force) {
    ...
    if (
      (typeof window === "undefined" || force === "node") &&
      force !== "browser"
    ) {
      // node implementation
      ...
      if (!(options?.controller instanceof ServiceWorkerRegistration))
        throw new Error("Invalid worker registration");
      if (options.controller.active.state !== "activated")
  ...
  get progress() {
    ...
    const length =
      torrents.reduce((total, torrent) => total + (torrent.length || 0), 0) ||
      1;
  ...
  get ratio() {
    ...
    const received =
      this.torrents.reduce((total, torrent) => total + torrent.received, 0) ||
      1;
  ...
  _destroy(err, cb) {
    ...
    if (this.natTraversal) {
      tasks.push((cb) => {
        this.natTraversal.destroy().then(() => cb());
      });
    }
    ...
    for (const torrent of this.torrents) {
      if (!torrent.infoHashHash) {
        torrent.infoHashHash = await hash(
          hex2arr("72657132" /* 'req2' */ + torrent.infoHash),
          "hex",
        );
      }
