 ...
 *
 * @param id "ip:port" string, peer id (for WebRTC peers), or url (for Web Seeds)
  ...
  onPe3(infoHashHash) {
    if (this.swarm) {
      if (this.swarm.infoHashHash !== infoHashHash) {
        this.destroy(
          new Error("unexpected crypto handshake info hash for this swarm"),
        );
      }
      this.sendPe4();
    }
...
Peer.TYPE_TCP_INCOMING = "tcpIncoming";
Peer.TYPE_TCP_OUTGOING = "tcpOutgoing";
Peer.TYPE_UTP_INCOMING = "utpIncoming";
Peer.TYPE_UTP_OUTGOING = "utpOutgoing";
Peer.TYPE_WEBRTC = "webrtc";
Peer.TYPE_WEBSEED = "webSeed";

Peer.SOURCE_MANUAL = "manual";
Peer.SOURCE_TRACKER = "tracker";
Peer.SOURCE_DHT = "dht";
Peer.SOURCE_LSD = "lsd";
Peer.SOURCE_UT_PEX = "ut_pex";
...
Peer.createTCPOutgoingPeer = (addr, swarm, throttleGroups, source) => {
  return Peer._createOutgoingPeer(
    addr,
    swarm,
    TYPE_TCP_OUTGOING,
    throttleGroups,
